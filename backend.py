...
# search function
# user will enter either 1 or all at same time
# will show all info regarding their choice
# pass empty string if user choice one subj to look for their stuff
# example title=' ', author=' ', etc
# will search empty title, or other info depending on user type
"""
def search(title='', author='', year='', isbn='', price='', quantity=''):
    conn = sqlite3.connect('books.db')
    cur = conn.cursor()
    cur.execute('SELECT * FROM book WHERE title=? OR author=? OR year=? OR isbn=? OR price=? OR quantity=?',
                (title, author, year, isbn, price, quantity))
    rows = cur.fetchall()
    conn.close()
    return rows
"""
